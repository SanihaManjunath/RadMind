...
const globe = Globe()
  ...
  /* Dash settings = moving arrow */
  .arcDashLength(0.15)        // size of arrowhead
  .arcDashGap(3.5)            // space between arrows
  .arcDashInitialGap(() => Math.random() * 5)
  .arcDashAnimateTime(2200)   // speed of movement
...
function focusOnAttack(a) {
  ...
  globe.pointOfView(
    { lat: a.endLat, lng: a.endLng, altitude: 1.6 },
    1200
  );
...
function updateCounters() {
  ...
  if (attacksThisMinute.length > peakPerMinute) {
    ...
    peakEl.style.color =
      peakPerMinute >= 20 ? "#ff4d4f" :
      peakPerMinute >= 10 ? "#faad14" : "#fff";
    pulse(peakEl);
  }
}
